...
ui <- dashboardPage(
    ...
    dashboardSidebar(
        ...
        textInput(inputId="group", label="Group", value = "", width = NULL, placeholder = "Manke"),
        textInput(inputId="user", label="User", value = "", width = NULL, placeholder = "Sikora"),
        textInput(inputId="projectid", label="ProjectID", value = "", width = NULL, placeholder = "A000"),
        selectInput(inputId="datatype", label="InputDataType", choices=c("bulkRNAseq","WGBS","scRNAseq"), selected ="bulkRNAseq", multiple = FALSE,selectize = TRUE, width = NULL, size = NULL),
...
server <- function(input, output) {
    ...
    fdv<-c("bulkRNAseq"="Please provide a one-column headerless txt file with ensemble gene IDs you would like to analyze.",
            "scRNAseq"="Please provide a one-column headerless txt file with ensemble gene IDs you would like to analyze.",
            "WGBS"="Please provide a bed file with the genomic intervals you would like to analyze. Please use ensembl chromosome naming style.")
    output$fileDescription<-renderText(expr=as.character(fdv[names(fdv) %in% input$datatype]))
...
##########################read/load processed data from a database##################
    ...
    ##placeholder: query database using group/user/projectid fields
    ##sampleSheet... from LIMS?
    inDat<-fread("/data/boehm/group/lawir/A214/FK.RNASeq.OUT_2FC/featureCounts/counts.txt",header=TRUE,sep="\t")
    ##render the head
    output$datHead<-renderTable(head(inDat),caption="Original unnormalized input data",caption.placement = getOption("xtable.caption.placement", "top"),width=500)

    ##placeholder: get genome and annotation version from a database
    org<-sub("_ensembl/.+.bam","",sub("/package.+/organisms/","",system('grep -m 1 \"/data/repository/organisms\" /data/boehm/group/lawir/A214/FK.RNASeq.OUT_2FC/featureCounts/LOG',intern=TRUE)))
    output$organism<-renderText(paste("Detected genome is ",org))
    annot<-sub("/genes.gtf.+.bam","",sub("/package.+/data/repository/organisms/.+/ensembl/","",system('grep -m 1 \"/data/repository/organisms\" /data/boehm/group/lawir/A214/FK.RNASeq.OUT_2FC/featureCounts/LOG',intern=TRUE)))
    ...
    output$inTabHead<-renderTable(head(inTab),caption="Input gene IDs",caption.placement = getOption("xtable.caption.placement", "top"))
        ...
        observeEvent(input$savetable, {
            ...
            output$barplot<-renderPlot({
                ...
                #output$debug<-renderTable(plotdata.SE)
...
##########################################################
    ...
    output$resultPanels<-renderUI({if(input$datatype=="bulkRNAseq")
                                    {myTabs<-list(tabPanel(title="InputData",
                                                      fluidPage(
                                                          fluidRow(
                                                              tableOutput("datHead"),
                                   ...
                                   else if(input$datatype=="WGBS")
                                    {myTabs<-list(tabPanel(title="CpG.Heatmap",
                                                      fluidPage(
                                                          box(plotOutput("plot1")),
                                                          box(title = "Controls",sliderInput("slider1", "Number of observations:", 1, 100, 50)),
                                                          box(title="Method Description",renderText("Methylation ratios per CpG position in the selected genomic interval are visualized. SNP-containing positions as well as positions with unsufficient read coverage were filtered out."))
                                                               )
                                                          ),
                                                  tabPanel(title="MethRatio.Dotplot",
                                                      fluidPage(
                                                          box(plotOutput("plot2")),
                                                          box(title = "Controls",sliderInput("slider2", "Number of observations:", 1, 100, 50)),
                                                          box(title="Method Description",renderText("Methylation ratios were aggregated per genomic interval as mean values.SNP-containing positions as well as positions with unsufficient read coverage were filtered out."))
                                                               )
                                                          )
                                                  )
                                     }
                                   else if(input$datatype=="scRNAseq")
                                    {myTabs<-list(tabPanel(title="Tsne",
                                                      fluidPage(
                                                          box(plotOutput("plot1")),
                                                          box(title = "Controls",sliderInput("slider1", "Number of observations:", 1, 100, 50)),
                                                          box(title="Method Description",renderText("Normalized counts were aggregated over selected genes."))
                                                               )
                                                          )
                                         )
